...
<x-app-layout>
    ...
    <form id="create-project" action="{{ localized_route('projects.store', $entity) }}" method="POST" novalidate>
        ...
        <x-translatable-input name="name" :label="__('Project name')" />
        ...
        <x-translatable-textarea name="goals" :label="__('Goals for consultation')" />

        <x-translatable-textarea name="impact" :label="__('Who will the project impact')" />

        <x-translatable-textarea name="out_of_scope" :label="__('What is this project not going to do?')" />
