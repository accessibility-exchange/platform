...
<x-app-layout>
    ...
    <form class="stack" id="create-project" action="{{ localized_route('projects.store-context') }}" method="post" novalidate x-data="{context: '{{ old('context', session('context')) ?? '' }}'}">
        ...
        <fieldset class="field @error('context') field--error @enderror stack">
            <legend class="h3">{{ __('Please indicate if this is a new project or a progress report for an existing project.') }}</legend>
            <x-hearth-radio-buttons name="context" :options="Spatie\LaravelOptions\Options::forArray(['new' => __('A new project'), 'follow-up' => __('A follow-up to a previous project (such as a progress report)')])->toArray()" :checked="old('context', session('context')) ?? ''" x-model="context" />
