<form class="stack" id="create-project" action="{{ localized_route('projects.store-context', $entity) }}" method="post" novalidate x-data="{context: '{{ old('context', session('context')) ?? '' }}'}">
    @csrf
    ...
    <fieldset class="field @error('context') field--error @enderror stack">
        ...
        <div class="field @error('ancestor') field--error @enderror stack" x-show="context == 'follow-up'">
            ...
            <x-hearth-select name="ancestor" :options="array_merge(['' => ''], $entity->projects->pluck('name', 'id')->toArray())" :selected="old('ancestor_id', session('ancestor_id'))" />
