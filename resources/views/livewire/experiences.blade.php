        ...
        @foreach ($experiences as $i => $experience)
            <li class="stack">
                ...
                <div class="field @error("work_and_volunteer_experiences.{$i}.start_year") field--error @enderror">
                    ...
                    <x-hearth-input type="number" pattern="[0-9]*" inputmode="numeric" min="1900" :max="date('Y')"
                        :id="$name . '_' . $i . '_start_year'" :name="$name . '[' . $i . '][start_year]'" :value="$experience['start_year'] ?? ''" />
