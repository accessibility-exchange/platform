<fieldset id="{{ $name }}">
    ...
    <div class="flex flex-wrap md:flex-nowrap gap-4 w-full">
        ...
        <div class="field stack @error($name) field--error @enderror">
            ...
            <x-hearth-select class="w-auto" :name="$name . '_month'" :options="$months" :required="$required" :disabled="$disabled" :aria-describedby="$name.'-hint'" :value="old($name.'_month')" wire:model="month" />
        </div>

        <div class="field stack @error($name) field--error @enderror">
            <x-hearth-label :for="$name . '_day'" :value="__('forms.label_day')" />
            <x-hearth-input class="w-20" type="number" :name="$name . '_day'" min="1" max="31" pattern="[0-9]*" inputmode="numeric" :required="$required" :disabled="$disabled" :aria-describedby="$name.'-hint'" :value="old($name.'_day')" wire:model.lazy="day" />
        </div>
    </div>
