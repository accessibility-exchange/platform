...
<x-app-wide-tabbed-layout>
    ...
    <div class="w-prose flex flex-col lg:grid grid-cols-3 gap-8 mt-12">
        <div class="col-start-1 col-end-2 flex flex-col gap-8">
            <div class="shadow-md bg-white px-6 py-8 rounded space-y-6">
        ...
        <div class="col-start-2 col-end-4 flex flex-col gap-8">
            <div class="shadow-md bg-white px-6 py-8 rounded space-y-6" x-data="copyLink">
                <h3>{{ __('Engagement details') }}</h3>
                @if($engagement->checkStatus('draft'))
                <p>{{ __('Please complete your engagement details so potential participants can know what they are signing up for.') }}</p>
                <div class="box stack bg-grey-2">
                    <p>{{ __('You have not finished adding your engagement details.') }}</p>
                    <p>
                        <a class="cta secondary" href="{{ localized_route('engagements.edit', $engagement) }}">{{ __('Edit engagement details') }}</a>
                    </p>
                </div>
                @elseif($engagement->checkStatus('published'))
                    <p>{{ __('Last updated on :date', ['date' => $engagement->published_at->translatedFormat('F j, Y')]) }}</p>
                    <div class="flex flex-col md:flex-row gap-6 md:gap-16 md:items-center">
                        <a class="cta secondary" href="{{ localized_route('engagements.edit', $engagement) }}">{{ __('Edit engagement details') }}</a>
                        <a href="{{ localized_route('engagements.show', $engagement) }}">{{ __('View') }} <span class="sr-only">{{ $engagement->name }}</span></a>
                        <div>
                            <button class="borderless" @click="copy"><x-heroicon-o-clipboard aria-hidden="true" role="presentation" />{{ __('Copy link to share') }}</button>
                        </div>
                        <div class="md:-ml-12" role="alert" aria-live="polite" x-show="alert" x-transition:enter.duration.0ms x-transition:leave.duration.500ms>
            ...
            <div class="shadow-md bg-white px-6 py-8 rounded space-y-6">
                ...
                <div class="flex flex-col md:flex-row gap-6 md:gap-16 md:items-center">
                    <p>{!! __(':count participants confirmed', ['count' => '<span class="h4">'.$engagement->confirmedParticipants->count().'</span><br />']) !!}</p>
            ...
            <div class="shadow-md bg-white px-6 py-8 rounded space-y-6">
                <h3>{{ __('Documents') }}</h3>
                <p>{{ __('This includes reports, contracts, or anything else you would like Consultation Participants to be able to access.') }}</p>
            </div>
        </div>
    </div>
    <script>
