    ...
    <form action="{{ localized_route('community-members.update', $communityMember) }}" method="POST" enctype="multipart/form-data" novalidate>
        ...
        <fieldset>
            <div class="field @error('picture') field--error @enderror">
                ...
                @livewire('image-uploader', ['name' => 'picture', 'image' => $communityMember->getMedia('picture')->first(), 'alt' => $communityMember->picture_alt ?? ''])
            ...
            <div class="field @error('picture_alt') field--error @enderror">
                ...
                <x-hearth-input name="picture_alt" :value="old('picture_alt', $communityMember->picture_alt)" />
        ...
        <fieldset>
            ...
            @if($communityMember->links)
                @foreach ($communityMember->links as $i => $link)
                <div class="field @error('links.' . $i . '.url') field--error @enderror">
                    <x-hearth-label for="links_{{ $i }}_url" :value="__('Link address')" />
                    <x-hearth-input id="links_{{ $i }}_url" name="links[{{ $i }}][url]" :value="old('links[' . $i . '][url]', $communityMember->links[$i]['url'])" hinted="links-hint" />
                ...
                <div class="field @error('links.' . $i . '.text') field--error @enderror">
                    ...
                    <x-hearth-input id="links_{{ $i }}_text" name="links[{{ $i }}][text]" :value="old('links[' . $i . '][text]', $communityMember->links[$i]['text'])" hinted="links-hint" />
        ...
        <fieldset>
            ...
            <div class="field @error('locality') field--error @enderror">
                ...
                <x-hearth-input type="text" name="locality" value="{{ old('locality', $communityMember->locality) }}" required hinted="address-hint" />
        ...
        <fieldset x-data="{ creator: '{{ old('creator') ?? $communityMember->creator }}' }">
            <legend>{{ __('Who’s creating this community member page?') }}</legend>
            <x-hearth-radio-buttons name="creator" :options="$creators" :selected="old('creator', $communityMember->creator)" x-model="creator" />
            <div class="field @error('creator_name') field--error @enderror" x-show="creator == 'other'">
                ...
                <x-hearth-input type="text" name="creator_name" value="{{ old('creator_name', $communityMember->creator_name) }}" />
            ...
            <div class="field @error('creator_relationship') field--error @enderror" x-show="creator == 'other'">
                ...
                <x-hearth-input type="text" name="creator_relationship" value="{{ old('creator_relationship', $communityMember->creator_relationship) }}" />
