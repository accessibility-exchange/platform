<x-app-wide-layout>
    ...
    <div class="with-sidebar">
        ...
        <div class="stack">
            ...
            <p>
                {{ __('We will notify you about being invited to engagements by contacting :contact_person.', ['contact_person' => $individual->support_person_name ? __('your support person, :name', ['name' => $individual->support_person_name]) : __('you')]) }}
                @if($individual->support_person_name)
            ...
            <ul>
                <li>{!! Str::inlineMarkdown($individual->primary_contact_point) !!}</li>
            @if($individual->alternate_contact_point)
                <li>{!! Str::inlineMarkdown($individual->alternate_contact_point) !!}</li>
            @endif
            </ul>
            ...
            <div class="field @error('preferred_notification_method') field--error @enderror">
                ...
                <x-hearth-select name="preferred_notification_method" :options="$notificationMethods" :selected="old('preferred_notification_method', $individual->preferred_notification_method)" :disabled="is_null($individual->alternate_contact_point)" />
            </div>

            <fieldset x-data="{notifyOfReports: {{ empty($individual->notifications['reports']['channels']) ? 0 : 1 }}}">
                ...
                <div class="field">
                    <x-hearth-radio-button name="notify_of_reports" value="1" :checked="old('notify_of_reports', empty($individual->notifications['reports']['channels']) ? 0 : 1) == 1" x-model.number="notifyOfReports" /> <x-hearth-label for="notify_of_reports-1">{{ __('Notify me') }}</x-hearth-label>
                </div>
                <div class="field__subfield stack" x-show="notifyOfReports == 1">
                    <x-hearth-checkboxes name="report_notifications" :options="$notificationChannels" :checked="old('report_notifications', $individual->notifications['reports']['channels'])" />
                </div>
                <div class="field">
                    <x-hearth-radio-button name="notify_of_reports" value="0" :checked="old('notify_of_reports', empty($individual->notifications['reports']['channels']) ? 0 : 1) == 0" x-model.number="notifyOfReports" /> <x-hearth-label for="notify_of_reports-0">{{ __('Don’t notify me') }}</x-hearth-label>
                </div>
            </fieldset>

            <h3 id="{{ Str::slug(__('Finding out about new projects')) }}">{{ __('Finding out about new projects') }}</h3>

            <div x-data="{notifyOfProjects: {{ empty($individual->notifications['projects']['channels']) ? 0 : 1 }}}">
                <fieldset>
                    <legend><h4>{{ __('Please indicate if you would like to be notified of new projects.') }}</h4></legend>
                    <div class="field">
                        <x-hearth-radio-button name="notify_of_projects" value="1" :checked="old('notify_of_projects', empty($individual->notifications['projects']['channels']) ? 0 : 1) == 1" x-model.number="notifyOfProjects" /> <x-hearth-label for="notify_of_projects-1">{{ __('Notify me') }}</x-hearth-label>
                    </div>
                    <div class="field__subfield stack" x-show="notifyOfProjects == 1">
                        <x-hearth-checkboxes name="project_notifications" :options="$notificationChannels" :checked="old('project_notifications', $individual->notifications['projects']['channels'])" />
                    </div>
                    <div class="field">
                        <x-hearth-radio-button name="notify_of_projects" value="0" :checked="old('notify_of_projects', empty($individual->notifications['projects']['channels']) ? 0 : 1) == 0" x-model.number="notifyOfProjects" /> <x-hearth-label for="notify_of_projects-0">{{ __('Don’t notify me') }}</x-hearth-label>
                    </div>
                </fieldset>
                ...
                <fieldset x-show="notifyOfProjects == 1">
                    ...
                    ]" :checked="old('project_creators', $individual->notifications['projects']['creators'])" />
                </fieldset>
                ...
                <fieldset x-show="notifyOfProjects == 1">
                    ...
                    ]" :checked="old('project_types', $individual->notifications['projects']['types'])" />
                </fieldset>
                ...
                <fieldset x-show="notifyOfProjects == 1">
                    ...
                    ]" :checked="old('project_engagements', $individual->notifications['projects']['engagements'])" />
                </fieldset>
            </div>

            <h3 id="{{ Str::slug(__('Keeping my information up to date')) }}">{{ __('Keeping my information up to date') }}</h3>

            <fieldset x-data="{notifyOfUpdates: {{ empty($individual->notifications['updates']['channels']) ? 0 : 1 }}}">
                ...
                <div class="field">
                    <x-hearth-radio-button name="notify_of_updates" value="1" :checked="old('notify_of_updates', empty($individual->notifications['updates']['channels']) ? 0 : 1) == 1" x-model.number="notifyOfUpdates" /> <x-hearth-label for="notify_of_updates-1">{{ __('Notify me') }}</x-hearth-label>
                </div>
                <div class="field__subfield stack" x-show="notifyOfUpdates == 1">
                    <x-hearth-checkboxes name="update_notifications" :options="$notificationChannels" :checked="old('updates_notifications', $individual->notifications['updates']['channels'])" />
                </div>
                <div class="field">
                    <x-hearth-radio-button name="notify_of_updates" value="0" :checked="old('notify_of_updates', empty($individual->notifications['updates']['channels']) ? 0 : 1) == 0" x-model.number="notifyOfUpdates" /> <x-hearth-label for="notify_of_updates-0">{{ __('Don’t notify me') }}</x-hearth-label>
                </div>
            </fieldset>
        </div>
    </div>
