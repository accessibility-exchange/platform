<form class="stack" action="{{ localized_route('organizations.update-constituencies', $organization) }}" method="POST"
    ...
    <div class="with-sidebar with-sidebar:last">
        ...
        <div class="stack" x-data="{ livedExperiences: [{{ implode(', ', old('lived_experiences', $organization->livedExperiences->pluck('id')->toArray() ?? [])) }}] }">
            ...
            <p><strong><em>{{ __('Primary constituency means a group that’s specifically in your mandate to :represent_or_serve_and_support.', ['represent_or_serve_and_support' => $organization->type === 'representative' ? __('represent') : __('serve and support')]) }}</em></strong>
            </p>

            @if ($organization->isConnector())
                <p><em>{{ __('As you’ve indicated that your organization is playing the role of Community Connector, a Regulated Organization may request your services to assist them in connecting to these groups.') }}</em>
                </p>
            @endif

            @if ($organization->isParticipant())
                <p><em>{{ __('As you’ve indicated that your organization is playing the role of Consultation Participant, a Regulated Organization may ask you to represent this group’s point of view in consultations.') }}</em>
                </p>
            @endif
            ...
            <fieldset class="field @error('indigenous_identities') field--error @enderror" x-data="{ hasIndigenousIdentities: '{{ old('has_indigenous_identities', $organization->extra_attributes->get('has_indigenous_identities', '')) }}' }">
                ...
                <div class="field">
                    <input id="has_indigenous_identities-1" name="has_indigenous_identities" type="radio"
                        value="1" @checked(old('has_indigenous_identities', $organization->extra_attributes->get('has_indigenous_identities', ''))) x-model="hasIndigenousIdentities" /> <label
                ...
                <div class="field">
                    <input id="has_indigenous_identities-0" name="has_indigenous_identities" type="radio"
                        value="0" @checked(!old('has_indigenous_identities', $organization->extra_attributes->get('has_indigenous_identities', ''))) x-model="hasIndigenousIdentities" /> <label
            ...
            <fieldset class="field @error('age_brackets') field--error @enderror" x-data="{ hasAgeBrackets: '{{ old('has_age_brackets', $organization->extra_attributes->get('has_age_brackets', '')) }}' }">
                ...
                <div class="field">
                    <input id="has_age_brackets-1" name="has_age_brackets" type="radio" value="1"
                        @checked(old('has_age_brackets', $organization->extra_attributes->get('has_age_brackets', ''))) x-model="hasAgeBrackets" /> <label
            ...
            <fieldset class="field @error('ethnoracial_identities') field--error @enderror" x-data="{ hasEthnoracialIdentities: '{{ old('has_ethnoracial_identities', $organization->extra_attributes->get('has_ethnoracial_identities', '')) }}', otherEthnoracialIdentity: {{ old('other_ethnoracial_identity', $organization->other_ethnoracial_identity) ? 'true' : 'false' }} }">
                ...
                <div class="field">
                    <input id="has_ethnoracial_identities-1" name="has_ethnoracial_identities" type="radio"
                        value="1" @checked(old('has_ethnoracial_identities', $organization->extra_attributes->get('has_ethnoracial_identities', ''))) x-model="hasEthnoracialIdentities" /> <label
                ...
                <div class="field">
                    <input id="has_ethnoracial_identities-0" name="has_ethnoracial_identities" type="radio"
                        value="0" @checked(!old('has_ethnoracial_identities', $organization->extra_attributes->get('has_ethnoracial_identities', ''))) x-model="hasEthnoracialIdentities" /> <label
