<x-app-layout>
    ...
    <form class="stack" action="{{ localized_route('organizations.save-roles', $organization) }}" method="post" novalidate>
        <fieldset class="field @error('type') field--error @enderror">
            ...
            <x-hearth-hint for="roles">{{ __('Please tell us what your organization would like to do here. You must pick at least one of these roles. You can always change this later.') }} <a target="_blank" href="{{ localized_route('about.for-community-organizations') }}">{{ __('Learn more about these roles') }}</a> </x-hearth-hint>
