    ...
    <x-auth-card>
        ...
        <form class="stack" method="POST" action="{{ localized_route('login-store') }}" novalidate>
            ...
            <div class="field">
                ...
                <x-hearth-label for="remember" :value="__('auth.label_remember_me')" />
            </div>
        ...
        @env('local')
            @foreach (App\Models\User::all() as $user)
                <x-login-link :key="$user->id" :label="$user->name . ' (' . App\Enums\UserContext::labels()[$user->context] . ')'" redirect-url="{{ localized_route('dashboard') }}" />
            @endforeach
        @endenv
    </x-auth-card>
</x-guest-layout>
