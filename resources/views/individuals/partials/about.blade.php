...
<h3 class="repel">{{ __('As a Community Connector, :name can connect to:', ['name' => $individual->firstName]) }} @can('update', $individual)<p><a class="cta secondary" href="{{ localized_route('individuals.edit', ['individual' => $individual, 'step' => 2]) }}">{!! __('Edit :section', ['section' => '<span class="visually-hidden">' . __('groups you can connect to') . '</span>']) !!}</a></p>@endcan</h3>
...
@if($individual->extra_attributes->has_indigenous_identities || $individual->extra_attributes->has_ethnoracial_identities)
...
@if($individual->extra_attributes->has_gender_and_sexual_identities || $individual->extra_attributes->has_refugee_and_immigrant_constituency)
...
@if($individual->connection_lived_experience === 'yes-all' || $individual->connection_lived_experience === 'yes-some')
    <h3>{{ __('Does :name have lived experience of the people they can connect to?', ['name' => $individual->firstName]) }}</h3>
    {{-- TODO: add attribute getter for this --}}
    <p>{{ App\Enums\CommunityConnectorHasLivedExperience::from($individual->connection_lived_experience)->labels()[$individual->connection_lived_experience] }}</p>
@endif
@endif
