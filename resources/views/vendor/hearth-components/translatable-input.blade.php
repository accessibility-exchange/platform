<div class="field @error($generateErrorId($name, locale())) field--error @enderror">
    ...
    <x-hearth-input type="text" :id="$generateFieldId($name, locale())" :name="$name . '[' . locale() . ']'" :value="old($generateErrorId($name, locale()), $model ? $model->getTranslation($name, locale()) : '')" />
    ...
    @if ($locale !== locale())
        <div class="expander field @error($generateErrorId($name, $locale)) field--error @enderror"
            ...
            <div class="expander__content" x-show="expanded">
                <x-hearth-input type="text" :id="$generateFieldId($name, $locale)" :name="$name . '[' . $locale . ']'" :value="old($generateErrorId($name, $locale), $model ? $model->getTranslation($name, $locale) : '')" x-model="value"
