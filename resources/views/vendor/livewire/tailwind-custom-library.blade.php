            ...
            <div class="flex w-full justify-between sm:hidden">
                <ul class="pagination flex w-full flex-row items-center justify-between" role="list">
                    <li>
                        ...
                        @else
                            <a href="{{ route(RalphJSmit\Livewire\Urls\Facades\Url::currentRoute(), ['library' => request()->route('library'), 'page' => $paginator->currentPage() - 1]) }}"
                        ...
                        @if (is_array($element))
                            @foreach ($element as $page => $url)
                                <li wire:key="paginator-{{ $paginator->getPageName() }}-page{{ $page }}">
                                    <a href="{{ route(RalphJSmit\Livewire\Urls\Facades\Url::currentRoute(), ['library' => request()->route('library'), 'page' => $page]) }}"
