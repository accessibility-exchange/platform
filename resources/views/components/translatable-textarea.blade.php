...
@foreach($locales as $locale)
    ...
    <div class="expander field @error($name . '.' . $locale) field--error @enderror" x-data="{expanded: false, value: '{{ old($name . '.' . $locale, '') }}'}">
        ...
        <div class="expander__content" x-show="expanded">
            <x-hearth-textarea :id="$name . '_' . $locale" :name="$name . '[' . $locale . ']'" :value="old($name . '.' . $locale, '')" x-model="value" :aria-labelledby="Str::slug(__(':label (:locale)', ['label' => $label, 'locale' => get_locale_name($locale)]))" />
