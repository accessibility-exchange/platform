<div class="field @error($name . '.' . locale()) field--error @enderror stack">
    ...
    <x-hearth-textarea :id="$name . '_' . locale()" :name="$name . '[' . locale() . ']'" :value="old($name . '.' . locale(), $model ? $model->getTranslation($name, locale(), false) : '')" />
...
@foreach($locales as $locale)
    ...
    <div class="expander field @error($name . '.' . $locale) field--error @enderror stack" x-data="{expanded: false, value: '{{ old($name . '.' . $locale, $model ? $model->getTranslation($name, $locale, false) : '') }}', badgeText: '{{ __('Content added') }}'}">
        ...
        <div class="expander__content" x-show="expanded">
            <x-hearth-textarea :id="$name . '_' . $locale" :name="$name . '[' . $locale . ']'" :value="old($name . '.' . $locale, $model ? $model->getTranslation($name, $locale, false) : '')" x-model="value" x-on:keyup="badgeText = '{{ __('Content added, unsaved changes') }}'" :aria-labelledby="Str::slug(__(':label (:locale)', ['label' => $label, 'locale' => get_locale_name($locale)]))" />
            <x-hearth-error :for="$name . '.' . $locale" />
        </div>
    </div>
    @endif
